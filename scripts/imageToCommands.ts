...
export async function findPathFromImage(params: ImagePathOptions) {
    ...
    for (const points of sortedSubClusters) {
        path += `M ${Math.round(options.offset + points[0][0] * scaling)} ${Math.round(options.offset/2 + (options.resolution - points[0][1]) * scaling)} `;
        for (let j = 0; j < points.length; j++) {
            ...
            if (j > 0 && Math.abs(points[j-1][1] - point[1]) <= 2) {
                path += `L ${Math.round(options.offset + point[0] * scaling)} ${Math.round(options.offset/2 + (options.resolution - point[1]) * scaling)} `;
            } else {
                path += `M ${Math.round(options.offset + point[0] * scaling)} ${Math.round(options.offset/2 + (options.resolution - point[1]) * scaling)} `;
            }
        }
    }
